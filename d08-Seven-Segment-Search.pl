...
# useful modules
use List::Util qw/sum uniq uniqstr any all/;
...
for my $line ( @input) {
    ...
    @output = split(" ", $out );
    for my $el (@output) {
	$count++ if (length($el) == 2 or length($el) == 3 or length($el) == 4 or length($el)==7);
    }
    my $sol = solve(@values);
    if (defined $sol) {
...
sub solve {
    ...
    my $ret = undef;
    my @v = @_;
    @v = sort {length($a) <=> length($b)} map {join("",sort split(//,$_))} @v;

    for my $per (@permutations) {
	my %p;
#	say join(',',@$per);
#	for my $k (keys %patterns) {
#	    my $pattern =$patterns{$k};
#	    say join(' ',map{$per->[$_]} @{$patterns{$k}});
#	    $p{$k} = join('',sort map {$per->[$_]} @{$patterns{$k}})
#	}
#	next unless ( $p{1} eq $v[0] and $p{7} eq $v[1] and $p{4} eq $v[2]);
	...
	while (@rest) {
		...
		if (@rest and join("",sort map{$per->[$_]} @{$patterns{$i}}) eq $rest[0]) {
		    $p{$i}=shift @rest
		}
	    }
	}
